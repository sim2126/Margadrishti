...
from parkiq.api.services import ParkIQService, UnknownZoneError
...
def segments_cii(
    ...
    zone: str | None = None,
    svc: ParkIQService = Depends(get_service),
) -> CiiMapResponse:
...
def trends(svc: ParkIQService = Depends(get_service)) -> TrendsResponse:
    return svc.zone_trends()


@router.get("/zones", tags=["deployment"])
def zones(svc: ParkIQService = Depends(get_service)) -> dict:
...
def deployment_plan(
    req: DeploymentPlanRequest, svc: ParkIQService = Depends(get_service)
) -> DeploymentPlanResponse:
    try:
...
def segment_detail(
    physical_id: str, svc: ParkIQService = Depends(get_service)
) -> SegmentDetail:

-- Margadrishti production serving schema (PostGIS). Idempotent: safe to re-run.
-- Tables are populated by margadrishti.db.serving.publish_all from the gold layer.
...
-- Row-level security: a session sets margadrishti.zone_scope to the officer's jurisdictions
...
CREATE POLICY zone_scope ON segments_dim
    USING (
        current_setting('margadrishti.zone_scope', true) IS NULL
        OR current_setting('margadrishti.zone_scope', true) = ''
        OR zone = ANY (string_to_array(current_setting('margadrishti.zone_scope', true), ','))
    );
